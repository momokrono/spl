...
- Being as standard as possible (minimal external dependencies): we want `spl` to be as standalone as possible. As for now, the only dependency is [stb](https://github.com/nothings/stb) for saving and loading various image types, and is bundled in the source code, so no need to download anything other than this repository.
