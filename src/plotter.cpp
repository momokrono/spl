...
namespace spl
{
    auto plotter::generate_labels(int const number_of_labels_x, int const number_of_labels_y, sf::Font const & font) const
    ...
    void plotter::show() const
    {
        ...
        sf::VertexArray function{sf::LinesStrip};   // TODO: aggiungere la possibilità di cambiare primitiva
                                                    // per poter fare linee tratteggiate, cerchi, tratto punto, ... */
        ...
        font.loadFromFile("arial.ttf"); // TODO: magari usare un font di sistema
        auto const [axes, labels] = generate_labels(number_of_labels_x, number_of_labels_y, font);
        while (window.isOpen())
        {
            sf::Event event;
